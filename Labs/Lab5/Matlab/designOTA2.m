...
OTA.M1.gm_ID = 15;
OTA.M3.gm_ID = 10;
% Getting CDD For Both input and load stage 
CDD__GM_NMOS = look_up(nch, 'GM_CDD', 'GM_ID', OTA.M1.gm_ID, 'VDS', OTA.M1.VDS);
CDD__GM_PMOS = look_up(pch, 'GM_CDD', 'GM_ID', OTA.M3.gm_ID, 'VDS', OTA.M3.VDS);
%Input Pair
tolarance=100;
Current_Gm=0; 
Prev_Gm=0;
...
% Tolarance 8% 
while tolarance>0.08
    Prev_Gm=Current_Gm;
    %calculating Gm While adding Parastics to Calculations 
OTA.M1.gm = specs.GBW * (specs.CL+CDD_NMOS+CDD_PMOS) * 2 * pi;
% After Completing the desgin without parastics we add the parstics that
% effect the bandwidth 
CDD_NMOS=(OTA.M1.gm/CDD__GM_NMOS);
CDD_PMOS=(OTA.M1.gm/CDD__GM_PMOS);
Current_Gm=OTA.M1.gm;
...
OTA.M1.L =min(L_vector( gm_gds_vector >= OTA.M1.gm_gds));
 %Complete the line to get the minimum L that gives gm/gds >= OTA.M1.gm_gds
...
OTA.M1.VGS =look_upVGS(nch, 'GM_ID', OTA.M1.gm_ID, 'VDS', OTA.M1.VDS, 'L',OTA.M1.L);
 %Complete the line to get the VGS of M1
OTA.M1.VG =OTA.M1.VGS + OTA.M5.VDS;
%Complete the line to get the DC CM input of OTA
% Calculating Tolarance 
tolarance= ((Current_Gm - Prev_Gm)/Current_Gm);
