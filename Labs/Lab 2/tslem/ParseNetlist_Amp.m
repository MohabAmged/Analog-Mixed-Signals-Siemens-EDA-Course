function [Node_1 Node_2 Node_C1 Node_C2 Values Names] = ParseNetlist_Amp(netlist, instances_key)

%{
Part 1: We loop on the netlist lines to search for the given instances' key
For each hit (instance found) we save the first, and second nodes,Conrtol Nodes, value,
and name
...
for line_number = 2:1:numel(netlist)
    ...
    if line(1) == upper(instances_key)
        ...
        Node_C1 =[Node_C1 splitted_line(4)];
        Node_C2 =[Node_C2 splitted_line(5)];
